...
export default function PaymentResult() {
  ...
  useEffect(() => {
    ...
    if (responseCode === "00") {
      // Step 1: Fetch order from backend to verify status
      verifyOrderPayment(orderId);
    } else {
      setStatus("failed");
      // Map common VNPay response codes to user-friendly messages
      switch (responseCode) {
        case "24":
          ...
          <>
            ...
            >
              ...
              <div style={{ display: "flex", justifyContent: "space-between", fontSize: "15px", fontWeight: "600" }}>
                ...
                <span>{formatCurrency(order?.totalAmount ?? vnpayAmount)}</span>
              </div>
            </div>
